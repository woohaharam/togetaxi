...
create table public.profiles (
  ...
  pay_link text check (char_length(pay_link) <= 200), -- 송금 링크 또는 계좌 안내
  created_at timestamptz not null default now()
);
...
begin
  if new.email is null or new.email !~* '\.(ac\.kr|edu)$' then
    raise exception '대학교 이메일(.ac.kr, .edu)로만 가입할 수 있어요';
  end if;
  return new;
end;
$$;
...
begin
  ...
  if not found then
    raise exception '프로필을 먼저 만들어 주세요';
  end if;
  return v;
end;
$$;
...
begin
  if p_depart_at < now() - interval '10 minutes' then
    raise exception '출발 시간이 이미 지났어요';
  end if;
  if p_depart_at > now() + interval '30 days' then
    raise exception '30일 이내의 일정만 올릴 수 있어요';
  end if;
  ...
  perform public.system_message(v_id, me.nickname || '님이 모집을 시작했어요');
  return v_id;
end;
$$;
...
begin
  ...
  perform public.system_message(p_ride, me.nickname || '님이 참여했어요');
end;
$$;
...
begin
  ...
  if r.host_id = me.id then
    raise exception '방장은 나갈 수 없어요. 공고를 취소해 주세요';
  end if;
...
begin
  ...
  if found then
    ...
    update public.rides set member_count = member_count - 1 where id = p_ride;
    perform public.system_message(p_ride, coalesce(v_name, '알 수 없음') || '님이 내보내졌어요');
  end if;
end;
$$;
...
begin
  ...
  perform public.system_message(p_ride, case p_status
    when 'open' then '모집을 다시 시작했어요'
    when 'closed' then '모집이 마감됐어요'
    when 'cancelled' then '방장이 공고를 취소했어요'
  end);
end;
$$;
...
begin
  ...
  perform public.system_message(p_ride,
    '택시비 ' || to_char(p_fare, 'FM999,999,999') || '원 · ' || r.member_count || '명 · 1인당 '
    || to_char(v_each, 'FM999,999,999') || '원을 방장에게 보내 주세요');
end;
$$;
